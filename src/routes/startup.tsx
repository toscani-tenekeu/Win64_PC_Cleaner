...
function StartupPage() {
  ...
  return (
    <PageShell eyebrow="Boot" title="Startup Manager" description="Registry startup entries are read from Windows. Disabled commands are preserved in SQLite so they can be restored later.">
        ...
        <ul className="divide-y divide-border">
          {entries.map((entry) => <li key={entry.id} className="flex items-center gap-4 px-4 py-3">
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              <div className="flex items-center gap-2"><span className="text-sm font-medium">{entry.name}</span><Badge variant="outline" className="text-[10px] capitalize">{entry.source}</Badge>{!entry.enabled ? <Badge variant="secondary" className="text-[10px]">stored in SQLite</Badge> : null}</div>
