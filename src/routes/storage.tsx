...
function StoragePage() {
  ...
  const drivesQuery = useQuery({ queryKey: ["drives"], queryFn: api.drives, enabled: hydrated });
  ...
  return (
    <PageShell
      ...
      actions={
        <Button size="sm" className="gap-1.5" disabled={scan.isPending} onClick={() => scan.mutate()}>
          <Search className="h-3.5 w-3.5" />
          {scan.isPending ? "Scanning..." : "Analyze folder"}
        </Button>
      }
    >
      <Card className="p-4">
      ...
      <Card className="p-5">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <h2 className="mt-1 text-base font-semibold">Live capacity by volume</h2>
          </div>
          <Badge variant="secondary" className="font-mono text-[10px]">Windows CIM</Badge>
        </div>
        <div className="flex flex-col gap-4">
          {drives.map((drive) => {
            const percent = drive.totalGB ? (drive.usedGB / drive.totalGB) * 100 : 0;
            return (
              <div key={drive.id} className="flex flex-col gap-2">
                <div className="flex flex-wrap items-baseline justify-between gap-2 text-sm">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-muted-foreground">{drive.mount}</span>
                    <span className="font-medium">{drive.label}</span>
                    <Badge variant="outline" className="font-mono text-[10px]">{drive.fs}</Badge>
                  </div>
                  <span className="font-mono text-xs text-muted-foreground">
                    {drive.usedGB.toFixed(1)} GB used - {drive.freeGB.toFixed(1)} GB free
                  </span>
                </div>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-1 text-base font-semibold">Scanned usage</h2>
            </div>
            {scan.data ? <Badge variant="secondary" className="font-mono text-[10px]">{scan.data.scannedFiles.toLocaleString()} files</Badge> : null}
          </div>
                ...
                return (
                  <li key={item.type} className="space-y-1.5">
                    <div className="flex items-center justify-between text-sm">
                      <span>{item.type}</span>
                      <span className="font-mono text-xs text-muted-foreground">{formatBytes(item.sizeBytes)} - {percent.toFixed(1)}%</span>
                    </div>
                    <Progress value={percent} className="h-1.5" />
                  </li>
                );
        ...
        <Card className="p-0">
          <div className="flex items-center justify-between border-b border-border p-4">
            <div>
              ...
              <h2 className="mt-1 text-base font-semibold">Top direct subfolders</h2>
            </div>
            {scan.data ? <Badge variant="secondary" className="max-w-48 truncate font-mono text-[10px]">{scan.data.root}</Badge> : null}
          </div>
          <ul className="divide-y divide-border">
            {largestFolders.map((folder) => (
              <li key={folder.path} className="flex items-center gap-4 px-4 py-3 text-sm">
                <div className="flex min-w-0 flex-1 flex-col">
                  <span className="truncate font-mono text-xs">{folder.path}</span>
                  <span className="text-[11px] text-muted-foreground">{folder.items.toLocaleString()} files counted</span>
                </div>
