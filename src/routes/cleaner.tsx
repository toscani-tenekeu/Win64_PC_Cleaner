...
function CleanerPage() {
  ...
  const scan = useQuery({ queryKey: ["cleaner-scan"], queryFn: api.cleanerScan, enabled: typeof window !== "undefined" });
  const categories = scan.data ?? [];

  useEffect(() => {
    if (categories.length && Object.keys(selected).length === 0) {
      setSelected(Object.fromEntries(categories.filter((category) => category.risk === "safe" && category.available).map((category) => [category.id, true])));
    }
  }, [categories, selected]);
    ...
    >
      ...
      <Card className="p-0">
        <div className="grid grid-cols-[auto_1fr_auto_auto_auto] items-center gap-4 border-b border-border px-4 py-2 text-[11px] uppercase tracking-wider text-muted-foreground">
          <span className="w-4" /><span>Category</span><span className="text-right">Files</span><span className="text-right">Size</span><span className="text-right">Risk</span>
        </div>
        <ul className="divide-y divide-border">
          {categories.map((category) => {
            const meta = riskMeta[category.risk];
            const Icon = meta.icon;
            return <li key={category.id} className="grid grid-cols-[auto_1fr_auto_auto_auto] items-start gap-4 px-4 py-3.5">
              <Checkbox checked={Boolean(selected[category.id])} disabled={!category.available || clean.isPending} onCheckedChange={(value) => setSelected((current) => ({ ...current, [category.id]: Boolean(value) }))} className="mt-0.5" />
              <div className="flex min-w-0 flex-col gap-1">
                <div className="flex flex-wrap items-center gap-2"><span className="text-sm font-medium">{category.name}</span>{category.requiresAdmin ? <Badge variant="outline" className="text-[10px]">admin</Badge> : null}{!category.available ? <Badge variant="secondary" className="text-[10px]">not found</Badge> : null}</div>
              ...
              <span className={`inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-[10px] font-medium ${meta.cls}`}><Icon className="h-3 w-3" /> {meta.label}</span>
            </li>;
          })}
