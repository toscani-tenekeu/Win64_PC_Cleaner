...
function OverviewPage() {
  ...
  const overview = useQuery({ queryKey: ["overview"], queryFn: api.overview, enabled: hydrated });
  ...
  return (
    <PageShell
      ...
      actions={
        <Link to="/cleaner" className="inline-flex items-center gap-2 rounded-md bg-primary px-3.5 py-2 text-sm font-medium text-primary-foreground transition hover:opacity-90">
                ...
                <div key={drive.id} className="flex flex-col gap-2">
                  <div className="flex items-baseline justify-between text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="secondary" className="font-mono text-[10px]">{drive.fs}</Badge>
                    </div>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
            ...
            {activity.map((item) => (
              <li key={item.id} className="flex items-start justify-between gap-4 py-2.5 text-sm">
                ...
                <div className="text-right">
                  <div className="font-mono text-xs text-muted-foreground">{new Date(item.createdAt).toLocaleString()}</div>
                  {item.freedBytes ? <div className="font-mono text-xs text-primary">-{formatBytes(item.freedBytes)}</div> : null}
                </div>
              </li>
            ))}
        ...
        <Card className="p-5">
          ...
          <ul className="mt-3 space-y-3 text-sm">
            <HealthRow label="Backend" value={health.data?.ok ? "Connected" : health.isPending ? "Connecting" : "Offline"} tone={health.data?.ok ? "success" : "destructive"} />
...
function QuickAction({
  ...
  return (
    <Link to={to} className="flex items-center gap-3 rounded-md border border-border bg-background/50 p-3 text-sm transition hover:border-primary/40 hover:bg-accent">
